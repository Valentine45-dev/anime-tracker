...
class AnalyticsService {
  ...
  getUserMetrics(userId: string): UserMetrics {
    ...
    return {
      ...
      clubsJoined: userEvents.filter(e => e.name === 'club_joined').length
    }
  }
  ...
  getSystemMetrics(): SystemMetrics {
    ...
    return {
      ...
      cacheHitRate: 0 // Will be calculated from real cache metrics when implemented
    }
  }
  ...
  private calculateAverageResponseTime(): number {
    const performanceEvents = this.events.filter(e => 
      e.name === 'performance_metric' && 
      e.properties?.metric === 'api_response_time'
    )
